...
const FeaturedPropertiesSection = () => {
  ...
  return (
    <section className="relative w-full py-16 md:py-32 bg-gradient-to-b from-background via-background to-background/80 overflow-hidden">
      ...
      <div className="relative z-10 w-full">
        ...
        <div className="text-center mb-12 px-4 sm:px-6">
          ...
          <h2 className="text-4xl sm:text-5xl font-bold text-foreground mb-4">Propriedades Exclusivas</h2>
          ...
          <div className="w-full px-4 sm:px-6 md:max-w-6xl md:mx-auto">
            ...
            {isVistaBella && showVideo && (
              <div className="mb-8 w-full">
                <div
                  className="relative w-full rounded-2xl md:rounded-3xl overflow-hidden border-2 border-primary/50 shadow-lg"
            ...
            <div className="space-y-6 mb-12">
              ...
              <div className="space-y-4">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {activeProperty.badge && (
                    <Badge className="bg-gradient-to-r from-primary to-cyan-500 text-white border-none px-3 py-1 text-sm">
                      ⭐ {activeProperty.badge}
                    </Badge>
                  )}
                </div>
                ...
                <div>
                  <h3 className="text-3xl md:text-5xl font-bold text-foreground">{activeProperty.name}</h3>
                  <p className="text-base text-muted-foreground mt-2">{activeProperty.typology}</p>
                </div>
              </div>

              {/* CARDS COM INFORMAÇÕES */}
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <div className="bg-gradient-to-br from-primary/10 to-cyan-500/10 border border-primary/20 rounded-lg p-4">
                  <p className="text-xs text-muted-foreground mb-1">Preço</p>
                  <p className="text-xl md:text-2xl font-bold text-emerald-400">{activeProperty.priceRange}</p>
                </div>

                <div className="bg-gradient-to-br from-primary/10 to-cyan-500/10 border border-primary/20 rounded-lg p-4">
                  <p className="text-xs text-muted-foreground mb-1">Área</p>
                  <p className="text-xl md:text-2xl font-bold text-blue-400">{activeProperty.areaRange}</p>
                </div>
                ...
                <div className="bg-gradient-to-br from-primary/10 to-cyan-500/10 border border-primary/20 rounded-lg p-4">
                  ...
                  <p className="text-xl md:text-2xl font-bold text-cyan-400">{activeProperty.images.length}</p>
                </div>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <Link to={`/imoveis/${activeProperty.slug}`} className="flex-1">
                  <Button className="w-full bg-gradient-to-r from-primary to-cyan-500 hover:from-primary/90 hover:to-cyan-500/90 text-white rounded-lg gap-2 h-12 text-base">
            ...
            {isVistaBella && activeProperty.images && activeProperty.images.length > 0 && !showVideo && (
              <div className="mb-12 w-full">
                <h4 className="text-lg font-semibold text-foreground mb-4">🖼️ Galeria de Imagens</h4>
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                  {activeProperty.images.map((img, idx) => (
                    <div
                      key={idx}
                      className="relative h-32 sm:h-40 md:h-48 rounded-lg overflow-hidden group border border-border/50"
                    >
                      <img
                        src={img}
            ...
            <div className="mt-12 pt-8 border-t border-border/30 space-y-6">
                ...
                {displayProperties.map((prop, idx) => {
                  ...
                  return (
                    <div
                      ...
                      className={`cursor-pointer relative h-32 sm:h-40 md:h-48 rounded-lg overflow-hidden group border-2 transition-all ${
                        idx === activeIndex
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:justify-between">
                <div className="flex items-center gap-2">
                  <button
                    onClick={handlePrev}
                    className="p-2 sm:p-3 rounded-full bg-muted/50 hover:bg-muted border border-border/50 text-foreground transition-all"
                  >
                    <ChevronLeft className="w-5 h-5" />
                  </button>
                  <button
                    onClick={handleNext}
                    className="p-2 sm:p-3 rounded-full bg-gradient-to-r from-primary to-cyan-500 text-white hover:shadow-lg hover:shadow-primary/50 transition-all"
                  >
                ...
                <div className="flex gap-2 justify-center">
                  {displayProperties.map((_, index) => (
                    <button
                      ...
                      className={`h-2 rounded-full transition-all ${index === activeIndex ? "w-8 bg-gradient-to-r from-primary to-cyan-500" : "w-2 bg-muted/50 hover:bg-muted"}`}
                ...
                <div className="flex items-center gap-3 justify-center sm:justify-end">
                  ...
                  <button
                    ...
                    className={`p-2 sm:p-3 rounded-full transition-all ${isAutoPlay ? "bg-gradient-to-r from-primary to-cyan-500 text-white" : "bg-muted/50 text-foreground border border-border/50 hover:border-primary/30"}`}
                  >
