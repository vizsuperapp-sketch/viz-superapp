...
serve(async (req) => {
  ...
  try {
    ...
    // Validate payload size (~5MB decoded)
        ...
        messages: [
          {
            ...
            content: [
              {
                type: "text",
                text: "Enhance this real estate photo: improve lighting, color balance, contrast, and make it look professional and inviting for a property listing. Keep it realistic and natural-looking.",
              },
              {
                type: "image_url",
                image_url: {
    ...
    if (!response.ok) {
      ...
      return new Response(JSON.stringify({ error: "Erro ao melhorar foto" }), {
        status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }

    const data = await response.json();
    const enhancedImage = data.choices?.[0]?.message?.images?.[0]?.image_url?.url || null;

    if (!enhancedImage) {
      return new Response(JSON.stringify({ error: "Não foi possível melhorar a foto" }), {
        status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
